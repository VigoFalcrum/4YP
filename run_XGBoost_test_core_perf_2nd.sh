...
overall_results="XGBoost_testing_perf_results_Threadripper.txt"
> "$overall_results"
...
for core in "${core_counts[@]}"; do    
    ...
    for (( i=0; i<num_configs; i++ )); do
        ...
        perf_output=$(OMP_NUM_THREADS=$core sudo -E perf stat -e cache-misses,cache-references,task-clock,instructions,cycles \
            python3 XGBoost_test_perf.py --test "$n_estimators_val" "$max_depth_val" "1" "$core" 2>&1)
        
        # Extract training time.
        # Assumes XGBoost_train_perf.py prints "Training time: <number> seconds"
        # training_time=$(echo "$perf_output" | grep -i "Latency:" | awk '{print $3}')
        training_time=$(printf '%s\n' "$perf_output" | sed -n 's/.*Latency: \([^)]*\).*/\1/p')
